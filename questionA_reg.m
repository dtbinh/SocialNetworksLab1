...
cc_reg = cc_reg';
figure;
continuous_cumulative_dist(cc_reg,190);
...
deg_cent = sum(reg_deg)/190;
clos_cent = sum(closeness(reg))/190;
bet_cent = sum(node_betweenness_faster(reg))/190;
eigen_cent = sum(eigencentrality(reg))/190;